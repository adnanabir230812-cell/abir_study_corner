...
const targetPrefix = '1.3 Effects of Pathogens';
const searchRoots = [
...
for (const root of searchRoots) {
  if (fs.existsSync(root)) {
    ...
    const matched = files.find(f => f.startsWith(targetPrefix) && f.endsWith('.pptx'));
    if (matched) {
      targetPath = path.join(root, matched);
      break;
    }
  }
}
...
if (!targetPath) {
  console.log(`Could not find at common paths. Searching recursively for prefix ${targetPrefix}...`);
  ...
  function searchFile(dir) {
    try {
      ...
      for (const item of items) {
        ...
        } else if (item.startsWith(targetPrefix) && item.endsWith('.pptx')) {
          return full;
        }
      }
    } catch(e) {}
...
if (!targetPath) {
  console.error(`FATAL: Could not locate ${targetPrefix}*.pptx anywhere on drives.`);
  process.exit(1);
}
...
const tempZip = path.join(scratchDir, 'temp_physiology.zip');
const tempDir = path.join(scratchDir, 'temp_physiology');
...
let mdContent = `# 1.3 Effects of Pathogens on Plant Physiology Lecture Slides Reference\n\n`;
...
const outputPath = path.join(scratchDir, 'physiology_slides_text.md');

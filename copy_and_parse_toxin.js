...
const targetFileName = '1.2 Toxin.pptx';
const searchRoots = [
  ...
  'C:\\Users\\Lenovo\\.gemini\\antigravity\\scratch\\toppr\\scratch' // Fallback
];
...
// If not found in hardcoded paths, let's look recursively
if (!targetPath) {
  console.log(`Could not find at common paths. Searching recursively for ${targetFileName}...`);
  // Try to find the directory E:\Abir study or F:\ or D:\
...
if (!targetPath) {
  console.error(`FATAL: Could not locate ${targetFileName} anywhere on E:, D:, F: or C: drives.`);
  process.exit(1);
}
...
const tempZip = path.join(scratchDir, 'temp_toxin.zip');
const tempDir = path.join(scratchDir, 'temp_toxin');
...
let mdContent = `# 1.2 Toxin Lecture Slides Reference\n\n`;
...
const outputPath = path.join(scratchDir, 'toxin_slides_text.md');

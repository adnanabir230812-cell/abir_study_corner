...
async function patchDatabase() {
  try {
    ...
    if (!course) {
      console.error('Course 3201 not found.');
      return;
    }

    // Find Section A and B
    ...
    if (!secA || !secB) {
      console.error('Section A or B not found.');
      return;
    }

    // Helper to find or create topics by name dynamically
    async function getTopic(sectionId, name) {
    ...
    // Topics we need
    const tEpi = await getTopic(secA.id, 'Epidemiology & Disease Triangle');
    const tControl = await getTopic(secA.id, 'Principles of Disease Control');
    
    const tRice = await getTopic(secB.id, 'Rice Diseases');
    const tSugar = await getTopic(secB.id, 'Sugarcane Diseases');
    const tPulse = await getTopic(secB.id, 'Pulse & Oilseed Diseases');
    const tWheat = await getTopic(secB.id, 'Wheat & Barley Diseases');
...
* **Bare Rachis:** Wind and rain disperse the dry powdery spores, leaving behind only the bare central axis (**rachis**) of the ear at harvest time.
...
*Source: Slide Notes (Section B - 5. Wheat and Pulse), Page 1 & 8. All Disease Differences*`
    ...
    console.log('\nStarting database patch inserting new questions...');
    let addedCount = 0;
    
    for (const qData of newQuestions) {
      // Check if question already exists by prefix to avoid duplicates
      const prefix = qData.questionText.substring(0, 50);
      ...
      if (existing) {
        console.log(`Question already exists: "${existing.questionText.substring(0, 60)}..."`);
        // Update its text and answer to make sure they are correct and have repeat brackets!
        existing.questionText = qData.questionText;
        existing.answerText = qData.answerText;
        await existing.save();
        console.log('  Updated question details.');
      } else {
        console.log(`Inserting question: "${qData.questionText.substring(0, 60)}..."`);
        ...
        addedCount++;
        console.log(`  Created question and progress row (ID: ${q.id})`);
      }
    }

    console.log(`\nSuccessfully processed new questions! Newly added: ${addedCount}`);

    // Verification and count
    const sectionIds = [secA.id, secB.id];
    const topics = await Topic.findAll({ where: { sectionId: sectionIds } });
    const topicIds = topics.map(t => t.id);
    
    const countA = await Question.count({
      include: [{
        model: Topic,
    ...
    const countB = await Question.count({
      include: [{
        model: Topic,
    ...
    console.log('\n================ DATABASE REPORT ================');
    console.log(`Plant Pathology Section A Questions: ${countA}`);
    console.log(`Plant Pathology Section B Questions: ${countB}`);
    console.log(`Total Plant Pathology Questions: ${countA + countB}`);
    console.log('=================================================');
